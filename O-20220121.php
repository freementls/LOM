...
function replace_offset_depths($old_value, $new_value, $offset, $depth_of_offset) {
	...
	foreach($old_value_offset_depths as $new_value_offset => $new_value_depth) {
	...
	//if($offset_adjust > 0) {
		...
		foreach($this->context as $context_index => $context_value) {
			...
			foreach($context_value[2] as $context2_index => $context2_value) {
				fatal_error('plz! tag array not done!');
			...
			foreach($this->context[$context_index][3] as $context3_index => $context3_value) {	
				...
				$depth = end($context_depth_array); // $depth is not used but this line checks the end of the array to see if we need to make changes
	...
	if(is_numeric($selector)) {
		...
		if(!isset($this->offset_depths[$selector]) ) {
			if($selector === 0) {
				...
				print('$selector, $this->offset_depths: ');O::var_dump_full($selector, $this->offset_depths);
				O::fatal_error('offset not in $this->offset_depths in get_parent_depth');
			}
		}
		return $this->offset_depths[$selector] - 1;
		/*$depth_to_match = $depth - 1;
		$offset = $selector;
		while($prev_result !== false && $this->offset_depths[$offset] > $depth_to_match) {
			$prev_result = prev($this->offset_depths);
		...
		print('$offset in get_parent_depth: ');var_dump($selector_matches);exit(0);
		if($prev_result === false) {
			
		}
		return $this->offset_depths[$offset];*/
	} else {
		O::fatal_error('unhandled $selector type in get_parent_depth; $selector: ');var_dump($selector);
	}
	print('$selector, $this->offset_depths: ');var_dump($selector, $this->offset_depths);
...
function new_($new_value, $selector = false) {
	...
	//	print('$expanded_LOM in new_: ');var_dump($expanded_LOM);
		...
		*/
		...
		/*
			...
			// proper context updating for new_() being written!
	...
	*/
		...
		O::fatal_error('checking new_ with a string selector');
		/*
...
function expand($code = false, $offset = 0, $offset_to_add = 0, $offset_depths = false, $mode = 'lazy') {
	...
	if(sizeof($offset_depths) === 1) { // may be counterintuitive to check for 1 before 0, but I'm sure there's an interesting mathematical implication to the need to do so
		...
		$depth_to_match = current($offset_depths); // take the second entry
	}
	...
	if($offset_of_matched_depth === NULL) { // assume we are in a tag?
		$offset_of_matched_depth = $offset_to_add;
	}
