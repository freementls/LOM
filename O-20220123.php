	...
	if($this->debug) {
		...
		foreach($this->offset_depths as $offset => $depth) {
			...
			if($this->code[$offset] !== '<') {
				...
				O::fatal_error('$this->offset_depths (in replace_offset_depths) should never point to anything other than &lt;');
			}
			$last_offset = $offset;
		}
	}
...
function adjust_offset_depths($offset, $offset_adjust, $included_array = false) {
	...
	if($this->debug) {
		...
		foreach($this->offset_depths as $offset => $depth) {
			...
			if($this->code[$offset] !== '<') {
				print('$this->code, $this->offset_depths, $offset, $depth: ');O::var_dump_full($this->code, $this->offset_depths, $offset, $depth);
				O::fatal_error('$this->offset_depths (in adjust_offset_depths) should never point to anything other than &lt;');
			}
			$last_offset = $offset;
		}
	}
