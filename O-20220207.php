...
function get($selector, $matching_array = false, $add_to_context = true, $ignore_context = false, $parent_node_only = false, $tagged_result = false) {
	...
	//if($add_to_context) {
		...
		*/
		...
		// debug
		if($this->debug && $normalized_selector === 'items') {
...
function set_attribute($attribute_name, $new_value, $selector) {
	...
	foreach($selector_matches as $index => $value) {
		...
		if(O::has_attribute($attribute_name, $string)) {
			...
			$attribute_offset = strpos($string, '>', 1) + $offset;
		}
...
function replace($old_value, $new_value, $offset = 0, $included_array = false) {
	...
	if($this->string_operation_made_a_change) {
		...
		foreach($this->context as $context_index => $context_value) {
			...
			print('$this->context[$context_index] before string replacing: ');var_dump($this->context[$context_index]);
			...
			print('$this->context[$context_index] after  string replacing: ');var_dump($this->context[$context_index]);
		}
...
function expand($code = false, $offset = 0, $offset_to_add = 0, $offset_depths = false, $mode = 'lazy') {
	// expand does less than get_tag_string() and so each may have their place: expand should be more complete but less efficient
	...
	/*if(!isset($offset_depths[$offset + $offset_to_add])) { // go to the previous entry in $this->offset_depths
		$depth_to_match = O::depth($offset + $offset_to_add);
	} else*/if(sizeof($offset_depths) === 1) { // may be counterintuitive to check for 1 before 0 and be flipping here, but I'm sure there's an interesting mathematical implication to the need to do so related to putting opening tag starts as well as text in the offset_depths
...
//		$contained_string = substr($code, $offset + $opening_whitespace_length, $offset_of_matched_depth - $offset_to_add - $offset - $opening_whitespace_length);
	...
	//$full_string = substr($code, $offset, $offset_of_matched_depth - $offset);
