...
function recursive_select($code, $offset_to_add = 0, $selector_piece_index = 0, $offset_depths = false, $parent_depth = -1) { // terrible nomenclature for these select functions?
	...
	if(sizeof($this->tagnames) > 1 && $this->selector_scopes[$selector_piece_index] !== 'direct') {
		print('$this->tagnames, $this->selector_piece_sets, $this->selector_scopes, $selector_piece_index, $this->selector_scopes[$selector_piece_index]: ');var_dump($this->tagnames, $this->selector_piece_sets, $this->selector_scopes, $selector_piece_index, $this->selector_scopes[$selector_piece_index]);
	...
	//if($match_any_tagname) {
		...
		$tagname_regex = substr($tagname_regex, 0, strlen($tagname_regex) - 1);
	}
	print('$tagname_regex in recursive_select(): ');var_dump($tagname_regex);
	// also add an offset to the preg_match_all (clip the start)
	...
	preg_match_all('/<(' . $tagname_regex . ')([^<>]*?)>/', $code, $matches, PREG_OFFSET_CAPTURE, $matching_offset);
	...
	foreach(O::LOM_operators as $operator) {
		if(strpos($selector, $operator) !== false) {
			print('$selector, $operator: ');var_dump($selector, $operator);
	...
	if($this->string_operation_made_a_change) {
		if($offset_adjust != 0) {
			...
			if(O::is_in_an_opening_tag($offset)) { // the replace is in an opening tag
				preg_match('/ (\w+)="([^"]{0,})"/is', $new_value, $attribute_matches, PREG_OFFSET_CAPTURE);
				if(($attribute_matches[0][0]) === strlen($new_value)) { // it's an attribute; then we know it's not simply text talking about XML attributes!
					...
					preg_match('/ (\w+)="([^"]{0,})"/is', $old_value, $attribute_matches, PREG_OFFSET_CAPTURE);
