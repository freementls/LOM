...
**Fixtures.** Generated by `gen_perf_fixture.php` (world/region/zone/entity). Default `perf_fixture.xml` ≈ 2.58 MB. Size suite: 1 MB, 100 MB, 1 GB. 20 GB is opt-in (`LOM_ALLOW_HUGE=1`) and was **not run** on this host (~15 GiB free of 38 GiB); see §5.4.
...
### 5.4 20 GB extrapolation (not measured)

Native 1 GB: ~44.8M opens, construct ~10.5 s, peak RSS ~5.0 GB during full `lomc`. Linear in opens/bytes would suggest ~20× → order **~3–4 min construct**, **~100 GB RSS** for indexes+mmap views—above this machine. Contiguous `memmove` on `set` would also scale with file size (~40 s if proportional to the ~2 s 1 GB text set). Treat 20 GB as requiring either a larger host or piece-local gap buffers (not claimed here).

`gen_perf_fixture.php 20GB` can write the file to disk without loading it into PHP; measurement needs `LOM_ALLOW_HUGE=1 ./bench_sizes.sh` on adequate hardware.
...
Apache License 2.0. Source: repository artifacts include `O.php`, `native/liblom`, `lomc`, `lomd`, language connectors (Python/C#/Go/Java), `bench_ablation.sh`, `bench_sizes.sh`, `gen_perf_fixture.php`, and `docs/RESULTS.md`. SPDX headers on `O.php` and `lom.h`. Timing tables in this draft are reproducible with the bench scripts on comparable hardware; 20 GB numbers in §5.4 are **extrapolations only**.
