...
**Fixtures.** Generated by `gen_perf_fixture.php` (world/region/zone/entity). Default `perf_fixture.xml` ≈ 2.58 MB. Size suite: 1 MB, 100 MB, 1 GB. 20 GB is opt-in (`LOM_ALLOW_HUGE=1`) and was **not run** on this host (~15 GiB free of 38 GiB); see extrapolation below.

### 5.2b 20 GB extrapolation (not measured)
