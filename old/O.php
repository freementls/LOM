...
function __construct($file_to_parse, $use_context = true, $array_blocks = false, $array_inline = false) {
  ...
  $this->must_check_for_self_closing = false;
	...
	//print('$this->must_check_for_comment: ');var_dump($this->must_check_for_comment);
...
function context_array($LOM_array = false) {
	...
	if(!is_array($LOM_array)) {
		if(is_numeric($LOM_array)) {
			$LOM_array = (int)$LOM_array;
			$LOM_array = O::get($LOM_array);
		} else {
	...
	while($piece_offset < strlen($piece)) {
		if($parsing_attribute_name) {
			if($piece[$piece_offset] === '@') {
				$this->required_attributes[O::query_decode($attribute_name_piece)] = false;
				$attribute_name_piece = '';
				$piece_offset++;
				continue;
			...
			if($piece[$piece_offset] === '@') {
				$this->required_attributes[O::query_decode($attribute_name_piece)] = O::query_decode($attribute_value_piece);
			...
			if($piece_offset === 0) {
				...
				$this->required_attribute_sets[] = $this->required_attributes;
				$this->required_attributes = array();
...
function select_old($selector, $matching_array = false, $offset_depths = false) {
	...
	foreach($selector_piece_sets as $selector_piece_set_index => $selector_piece_set) {
		...
		foreach($selector_piece_set as $piece_index => $piece) {
			...
			while($piece_offset < strlen($piece)) {
				...
				if($parsing_attribute_name) {
					...
					if($piece_offset === 0) {
						...
						$required_attribute_sets[] = $this->required_attributes;
						$this->required_attributes = array();
						$piece_offset++;
						continue;
					}
			...
			//	$this->required_attributes[O::query_decode($attribute_name_piece)] = O::query_decode($attribute_piece);
			//}
...
function LOM_match_by_tagname($tagname, $matching_array, $look_only_in_direct_children = true, $tagvalue = false, $matching_index = false) {
	...
	//print('$tagname, $look_only_in_direct_children, $tagvalue, $matching_index, $this->required_attributes in match_by_tagname: ');var_dump($tagname, $look_only_in_direct_children, $tagvalue, $matching_index, $this->required_attributes);
	if(sizeof($matching_array) > 0) {
		foreach($matching_array as $index2 => $value2) {
			...
			foreach($value2 as $index => $value) {
				...
				$this->required_attributes_exist = true;
				if($value[0] == 1) {
					...
					foreach($this->required_attributes as $required_attribute_name => $required_attribute_value) {
						$this->required_attributes_exist = false;
						...
						if(!$this->required_attributes_exist) {
							break;
						}
					}
				} else {
					if(sizeof($this->required_attributes) > 0) {
						$this->required_attributes_exist = false;
					}
				}
				//if(sizeof($this->required_attributes) > 0) {
				//	print('$value, $this->required_attributes, $existing_attributes, $this->required_attributes_exist: ');var_dump($value, $this->required_attributes, $existing_attributes, $this->required_attributes_exist);
				//}
				...
				foreach($tagvalue_array as $tagvalue_index => $tagvalue) {
					...
					//print('here237541<br />');
					if(($look_only_in_direct_children === false || $matching_depth === 1) && $value[0] == 1 && $value[1][2] === 0 && ($tagname === $value[1][0] || $tagname == '*') && ($tagvalue === false || (strlen($tagvalue) > 0 && $value2[$index + 1][0] === 0 && $tagvalue === $value2[$index + 1][1])) && $this->required_attributes_exist) {
						//print('here237542<br />');
...
function get_all_named_tags($string, $tagName, $offset = 0, $tagvalue = false, $matching_index = false) {
	if(strpos($string, $tagName) === false && strpos($tagName, $string) !== false) { // swap them
		$temp_string = $tagName;
		$string = $tagName;
		$tagName = $temp_string;
	...
	//print('$string, $tagName, $offset, $tagvalue, $matching_index, $this->required_attributes in get_all_named_tags: ');var_dump($string, $tagName, $offset, $tagvalue, $matching_index, $this->required_attributes);
...
function get_all_tags($string, $offset = 0, $tagvalue = false, $matching_index = false) {
	/*
	//$string = substr($string, $offset); // bad
	$arrayOStrings = array();
	// it should be mentioned that "tags" such as CDATA regions will not be caught since we are only looking for words rather than non-spaces
...
function get_all_tags_at_this_level($string, $offset = 0, $tagvalue = false, $matching_index = false) {
	if(is_string($matching_index)) {
		$matching_index = (int)$matching_index;
	}
	$LOM = O::LOM($string);
...
function get_all_named_tags_at_this_level($string, $tagname, $offset = 0, $tagvalue = false, $matching_index = false) {
	if(is_string($matching_index)) {
		$matching_index = (int)$matching_index;
	}
	$LOM = O::LOM($string);
